...
describe("resolvePgConnectionConfig", () => {
  ...
  it("leaves localhost URLs without TLS when no CA or sslmode is set", () => {
    const config = resolvePgConnectionConfig("postgresql://user:pass@localhost:5432/db");
  ...
  it("enables TLS when sslmode=require is present without a CA", () => {
    const config = resolvePgConnectionConfig(
      "postgresql://user:pass@host:5432/db?sslmode=require"
    );
