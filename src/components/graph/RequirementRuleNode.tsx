...
export const RequirementRuleNode = memo(({ data }: NodeProps & { data: DegreeGraphNodeData }) => {
  ...
  const summary = [
    ...
    rule.minimumCourseLevel &&
      rule.maximumCourseLevel &&
      `Courses numbered ${rule.minimumCourseLevel}–${rule.maximumCourseLevel}`,
    rule.explicitCourseCodes?.length && `Options: ${rule.explicitCourseCodes.join(", ")}`,
  ].filter(Boolean);

  return (
    <div
      className="w-[300px] rounded-lg border-2 border-dashed bg-surface-container-lowest p-3 shadow-sm"
      style={{ borderColor: data.groupCategory === "elective" ? "#d97706" : "#747683" }}
    >
      ...
      <ul className="mt-2 space-y-0.5 text-[11px] text-on-surface-variant">
        {summary.slice(0, expanded ? undefined : 3).map((line) => (
          <li key={line}>• {line}</li>
        ))}
      </ul>
      {data.sourceText && (
        <>
          ...
          {expanded && (
            <p className="mt-2 whitespace-pre-wrap text-[11px] leading-relaxed text-on-surface-variant">
              {data.sourceText}
            </p>
          )}
        </>
