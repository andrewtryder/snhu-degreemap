...
export function MetricCard({ label, value, subtext, icon, variant = "default", className = "" }: MetricCardProps) {
  const variantStyles = {
  ...
  return (
    ...
    >
      ...
      {icon && <div className="rounded-lg bg-surface-container p-2.5 text-primary shrink-0">{icon}</div>}
    </div>
  );
}
