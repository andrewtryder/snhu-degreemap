...
export async function POST(request: Request) {
  ...
  if (!secret) {
    return NextResponse.json({ error: "Server misconfiguration: REVALIDATE_SECRET is missing" }, { status: 500 });
  }

  const authHeader = request.headers.get("authorization");
  const token = authHeader?.replace(/^Bearer\s+/i, "") ?? request.headers.get("x-revalidate-secret");
  ...
  try {
    ...
    return NextResponse.json({ error: `Revalidation error: ${(err as Error).message}` }, { status: 500 });
  }
}
